...
## Windows TZ ID ↔ IANA/Olson Mapping (Common Zones)

This is the authoritative mapping for .NET TimeZoneInfo (Windows IDs) and NodaTime/cross-platform (IANA IDs).
...
| UAE Standard Time | Arabian Standard Time | Asia/Dubai | +04:00 | No |
...
```
// TZDB (IANA) - recommended for cross-platform
DateTimeZoneProviders.Tzdb["Asia/Colombo"]

// BCL (Windows) - maps to TimeZoneInfo
DateTimeZoneProviders.Bcl["Sri Lanka Standard Time"]
```

## TimeZoneConverter NuGet Package

The `TimeZoneConverter` package (by Matt Johnson-Pint) is the standard bridge between Windows IDs and IANA IDs:
...
// Returns: "Sri Lanka Standard Time"

// Get TimeZoneInfo cross-platform (works on both Linux & Windows)
